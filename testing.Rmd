...
Upload the necessary R libraries that are required for the complete analysis.
...
dim(TestSet)
```
Datasets have 160 variables and have plenty of NA variables, that can be removed with the cleaning procedures below. The Near Zero variance (NZV) variables are also removed and the ID variables as well.
...
NoNA    <- sapply(TrainSet, function(x) mean(is.na(x))) > 0.95
TrainSet <- TrainSet[, NoNA==FALSE]
TestSet  <- TestSet[, NoNA==FALSE]
dim(TrainSet)
dim(TestSet)
...
```
```{r randomforest_plot, echo=FALSE}
# plot matrix results
...
```{r Decision_Trees, echo=FALSE}
# model fit
set.seed(12345)
modFitDecTree <- rpart(classe ~ ., data=TrainSet, method="class")
fancyRpartPlot(modFitDecTree)
```
```{r Decision_Trees_test}
# prediction on Test dataset
predictDecTree <- predict(modFitDecTree, newdata=TestSet, type="class")
confMatDecTree <- confusionMatrix(predictDecTree, TestSet$classe)
confMatDecTree
```
```{r Decision_Trees_matrix, echo=FALSE}
# plot matrix results
